...
class TaskStore:
    ...
    async def create_task(self, payload: TaskCreateRequest) -> TaskDetail:
        ...
        task.events.append(TaskEvent(at=now, level="info", message="任务已创建，等待执行"))
    ...
    async def mutate_task(self, task_id: str, mutator: Callable[[TaskDetail], None]) -> TaskDetail | None:
        async with self._lock:
            task = self._tasks.get(task_id)
            if not task:
                return None
